...
Для этого в качестве обработчика события передайте компоненту параметр, полученный от родителя, следующим образом:
...
Наконец, ваш компонент `Button` принимает свойство `onClick`. Он передает этот параметр непосредственно встроенному браузерному `<button>` с `onClick={onClick}`. Это указывает React на вызов переданной функции по нажатию мыши.
...
Встроенные компоненты, такие как `<button>` и `<div>`, поддерживают только [имена событий браузера](/reference/react-dom/components/common#common-props), например `onClick`. Однако при создании собственных компонентов вы можете именовать параметры обработчиков событий как угодно.

По соглашению, параметры обработчика событий должен начинаться с `on`, за которым следует заглавная буква.

Например, параметр `onClick` компонента `Button` можно было бы назвать `onSmash`:
...
В этом примере `<button onClick={onSmash}>` показывает, что браузеру `<button>` (в нижнем регистре) все еще нужен параметр `onClick`, но имя параметра, полученное вашим пользовательским компонентом `Button`, зависит от вас!

Если ваш компонент поддерживает несколько взаимодействий, вы можете именовать параметры обработчиков событий для концепций, специфичных для конкретного приложения. Например, этот компонент `Toolbar` получает обработчики событий `onPlayMovie` и `onUploadImage`:
